...
@Component({
  selector: 'app-territorywupie',
  templateUrl: './territoryWU.component.html',
  styleUrls: ['../myreport.component.scss'],
  encapsulation: ViewEncapsulation.None
})
export class TerritoryWUComponent implements OnInit, AfterViewInit {
  ...
  ngOnInit() {
    ...
    this.apiService.getReports(pdata).subscribe(data=>{
      ...
      result.forEach(element => {
        result_arr_data.push({name:element.Region,y:element.NetPaidClaimAmt});
      });
      result_arr.data=result_arr_data;

      console.log("result_arr.data==>",JSON.stringify(result_arr_data));

      result_arr.title="Territory Wise Utilization";
      result_arr.subtitle="";
      result_arr.yAxis="RELATION";
      result_arr.name="Region";
