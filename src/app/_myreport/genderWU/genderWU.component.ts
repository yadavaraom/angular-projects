...
@Component({
  selector: 'app-genderwupie',
  templateUrl: './genderWU.component.html',
  styleUrls: ['../myreport.component.scss'],
  encapsulation: ViewEncapsulation.None
})
export class GenderWUComponent implements OnInit, AfterViewInit {
  ...
  ngOnInit() {
    ...
    this.apiService.getReports(pdata).subscribe(data=>{
      ...
      result.forEach(element => {
        result_arr_data.push({name:element.GENDER,y:element.NetPaidClaimAmt});
      });
      result_arr.data=result_arr_data;

      console.log("result_arr.data==>",JSON.stringify(result_arr_data));

      result_arr.title="Gender Wise Utlization";
      result_arr.subtitle="";
      result_arr.yAxis="GENDER";
